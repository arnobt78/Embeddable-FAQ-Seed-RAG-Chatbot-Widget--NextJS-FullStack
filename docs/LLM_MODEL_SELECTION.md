...
> **Last verified: 2026-08-19** (web-search-verified against provider docs and independent trackers on this date). Free tiers change frequently — providers add credit-card requirements, cut rate limits, or deprecate models with little notice. Treat every row below as "true as of last verification," not a permanent guarantee. Re-check a provider's own pricing/docs page before depending on it in production.
>
...
| **GroqCloud** 🔑 | **Yes** — no card | `openai/gpt-oss-120b`, `openai/gpt-oss-20b`, `qwen/qwen3.6-27b` | Extremely fast inference (LPU hardware). See [deprecation watch](#groq-deprecation-watch-verified-2026-08-01-primary-source) below — `llama-3.3-70b-versatile` shutdown **2026-08-16** (past). ~14,400 req/day, 30 RPM typical on free tier (varies per model). |
...
**This repo (portfolio-chatbot-widget):** Implemented at `lib/ai/providers.ts` with Groq chain `openai/gpt-oss-120b` → `qwen/qwen3.6-27b` → `openai/gpt-oss-20b` (llama models removed after 2026-08-16 shutdown).
...
This repository (portfolio-chatbot-widget) implements this pattern at `lib/ai/`:

- **`providers.ts`** — provider registry (Layer 1): Gemini, OpenRouter `:free`, Groq OSS, Hugging Face router, OpenAI paid last.
- **`normalize-messages.ts`** — message content normalization for multi-format chat history.
- **`retriable.ts`** — retriable/rate-limit error classification (Layer 3 helper).
- **`gemini-stream.ts`** — Gemini native streaming (primary free-tier provider).
- **`openai-stream.ts`** — OpenAI-compatible streaming via Vercel AI SDK.
- **`index.ts`** — orchestrator (Layer 3): outer provider loop, inner model loop, 429 fast-skip.
- **`lib/ai.ts`** — re-export preserving the existing `@/lib/ai` import path.

Embeddings fallback chain lives in `lib/embeddings.ts` (Gemini → HF router → OpenRouter free → OpenAI).
